...
class ADUC:
    ...
    def Show(self):
        ...
        while True:
            ...
            if str(ret) == 'abort' or str(ret) == 'cancel':
                ...
                if choice == 'Users':
                    UI.ReplaceWidget('rightPane', self.__users_tab())
                    UI.ReplaceWidget('new_but',  MenuButton(Id('new'), "New", [Item(Id('new_user'), 'User'), Item(Id('new_group'), 'Group')]))
    ...
    def __computer_tab(self):
        items = self.__computer_tab_data()
        return Table(Id('comp_items'), Opt('notify'), Header('Name', 'Type', 'Description'), items)

    def __aduc_tree(self):
        return VBox(
                Tree(Id('aduc_tree'), Opt('notify'), 'Active Directory Users and Computers', [
